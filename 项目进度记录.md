...
| 阶段 | 内容 | 预计时间 | 状态 |
|:---:|:---|:---:|:---:|
| **Phase 0** | 环境搭建与数据准备 | 1周 | ✅ 已完成 |
| **Phase 1** | A0 基准模型实现 | 3-4天 | ✅ 已完成 |
| **Phase 2** | A1 参考图Cross-Attention | 3-4天 | 🔄 进行中 |
...
| 序号 | 任务 | 状态 | 完成日期 | 备注 |
|:---:|:---|:---:|:---:|:---|
| 0.1 | 确认BBDM原始代码可运行 | ✅ | 2026-02-04 | 运行Template-BBDM.yaml |
...
- [x] `python main.py --config configs/Template-BBDM.yaml --train` 可正常启动
- [x] DataLoader输出shape正确：GT[B,3,256,256], lineart[B,1,256,256]
- [x] 线稿提取质量可接受（边缘清晰，无过多噪声）
...
| 序号 | 任务 | 状态 | 完成日期 | 备注 |
|:---:|:---|:---:|:---:|:---|
| 1.1 | 修改UNet in_channels=4 | ✅ | 2026-02-04 | openaimodel.py |
| 1.2 | 修改BrownianBridgeModel前向过程 | ✅ | 2026-02-04 | 支持concat lineart |
| 1.3 | 创建A0配置文件 | ✅ | 2026-02-04 | configs/A0-Baseline.yaml |
| 1.4 | 训练A0模型 | ✅ | 2026-02-04 | ~50 epochs |
| 1.5 | 评估A0指标 (FID, LPIPS, PSNR) | ✅ | 2026-02-04 | 记录基准 |
| 1.6 | 保存A0可视化结果 | ✅ | 2026-02-04 | 选10张典型样本 |
...
- [ ] A0模型训练收敛，loss稳定下降
- [ ] 生成结果具有基本的着色能力
- [ ] 记录基准指标：FID=16.87, LPIPS=0.16, PSNR=18.50, SSIM=0.83
...
| 新增组件 | 说明 |
|:---|:---|
| ReferenceEncoder | 4层CNN，输出[B,256,256] (序列格式) |
| CrossAttention | 复用attention.py，UNet Middle Block内置 |

### 4.2 任务清单

| 序号 | 任务 | 状态 | 完成日期 | 备注 |
|:---:|:---|:---:|:---:|:---|
| 2.1 | 实现ReferenceEncoder类 | ✅ | 2026-02-04 | model/BrownianBridge/ReferenceEncoder.py |
| 2.2 | 实现ColorCrossAttention类 | ✅ | 2026-02-04 | 复用attention.py (已支持3维/4维context) |
| 2.3 | 修改UNet添加Cross-Attention | ✅ | 2026-02-04 | use_spatial_transformer=True启用 |
| 2.4 | 修改Dataset返回ref图像 | ✅ | 2026-02-04 | distorted字段，A1阶段ref=GT |
| 2.5 | 创建A1配置文件 | ✅ | 2026-02-04 | configs/A1-CrossAttn.yaml |
| 2.6 | 训练A1模型 | ⬜ | - | 待训练 |
| 2.7 | 评估A1指标并对比A0 | ⬜ | - | 待评估 |

### 4.3 验收标准

- [x] Cross-Attention正确注入，shape匹配
- [ ] 参考图颜色能有效迁移到生成结果
- [ ] A1指标优于A0
...
| 文件 | 功能 | 状态 |
|:---|:---|:---:|
| dataset/custom.py (AnimeColorizationDataset) | 数据集类 | ✅ |
| model/BrownianBridge/ReferenceEncoder.py | 参考图编码器 | ✅ |
| model/BrownianBridge/base/modules/attention.py | 交叉注意力 (已修改支持3维context) | ✅ |
| model/losses/edge_loss.py | 边缘感知损失 | ⬜ |
| dataset/tps_warp.py | TPS变形增强 | ✅ (已存在) |
